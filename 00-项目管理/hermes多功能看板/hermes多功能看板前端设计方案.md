...
| 项目 | 说明 |
|:----|:------|
| **布局** | 四列 Kanban（待办 / 进行中 / 已完成 / 已归档），左导航切换 |
| **筛选** | 顶部筛选栏：项目下拉 + 负责人下拉 + 状态筛选 + 筛选按钮 |
| **任务卡片** | ID、标题、优先级 Badge、项目名，hover 上浮效果 |
| **拖拽** | 使用 SortableJS（H5）或 `@uni-helper/sortablejs`（小程序） |
| **状态颜色** | 待办（灰）/ 进行中（靛蓝紫）/ 已完成（绿）/ 已归档（暗灰，半透明） |
...
- **状态 C「Skill」**：展示当前 profile 下注册的可用 skills 一览
...
Skill 页面展示当前 Hermes profile 下注册的所有可用 skills，参考 cronjob 的切换栏位置：

- **卡片网格布局**：每张卡片展示一个 skill 的概览信息
- 显示：skill 名称、简短描述、标签（tags）、分类（category）、版本号
- 卡片支持 hover 展开效果，展示更详细的描述和关联 skill 信息
- 顶部提供搜索框和分类筛选，便于快速定位 skill
- 支持按名称搜索 / 按 category 分类筛选
...
| `/pages/agents/agents` | Agent（扩展） | 增加 CronJob + Skill 子视图切换 |
...
- 🆕 Agent 页签下「状态 | CronJob | Skill」切换 bar
- 🆕 Git 仓库信息页面内容
- 🆕 里程碑进度一览页面内容
- 🆕 CronJob 状态页面内容
- 🆕 Skill 信息展示页面内容（skill 卡片网格 + 搜索筛选）
...
| Agent 页签二级切换栏 | agents.vue 顶部切换 bar + CronJob + Skill 视图 | P0 | M4.1 |
| Git 仓库信息页面开发 | 仓库列表、分支信息、提交记录展示 | P0 | M4.2 |
| Skill 信息页签 | Agent 页签下新增 skill 信息子视图 | P0 | M4.3 |
