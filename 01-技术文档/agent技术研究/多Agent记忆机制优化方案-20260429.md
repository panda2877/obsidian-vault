...
| L3 | 开发 skill-index skill | 实现三层匹配逻辑（L1 category/L2 trigger_scenes&keywords/L3 语义），输出推荐 skill 列表 | 待落地 |
...
#### L3 实施框架：skill-index skill

```
输入：任务描述（自然语言）
输出：推荐 skill 列表（top 1-3），含 name/owner/匹配理由

实现方式：
- 作为 Hermes skill 实现，加载时读取 skills-index.json
- 三层匹配：
  L1: 精确匹配 category
  L2: 匹配 trigger_scenes / keywords（中文优先）
  L3: 语义匹配 purpose 描述
- 返回结构：{"recommended": [{"name": "...", "owner": "...", "reason": "..."}]}
```
